...
// these arguments come from the dirent struct of readdir()
bool is_same_file(char first_file_path_from_archive[MAXPATHLEN], char second_file_path_from_archive[MAXPATHLEN]) {
  if (strcmp(first_file_path_from_archive, second_file_path_from_archive) == 0) {
    return true;
  } else {
    return false;
  }
